# CATopalian_PY_TOC_and_Book_Creator_for_MD_target.py

from pathlib import Path

# Configuration
ROOT_DIR = Path(".")
TARGET_DIR = ROOT_DIR / "code_book"  # Only scan this finalized folder
...
def get_sorted_python_files(base_path: Path):
    ...
    # Ensure the target directory actually exists before scanning
    if not base_path.exists():
        print(f"Warning: The directory '{base_path}' does not exist yet.")
        return []
...
def generate_table_of_contents(files):
    ...
    for file_path in files:
        ...
        # Relative path formatted with web forward-slashes (uses ROOT_DIR to build the correct link)
...
def main():
    print(f"Scanning directory structure in '{TARGET_DIR}'...")
    files = get_sorted_python_files(TARGET_DIR)
    
    if not files:
        print("No Python files found. Ensure your polished scripts are inside the 'code_book' folder.")
        return
